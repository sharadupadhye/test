...
try:
    # IMPORTANT: pymodbus uses zero-based addressing.
    # To read "register 2121" (documented), request address=2120.
    temp_addr = 2098 - 1
    rh_addr   = 2103 - 1
    ...
    if temp_rr.isError() or rh_rr.isError():
        ...
        # show both raw and ModScan style address for clarity
        print("Raw register values:")
        print(f"  Requested (pymodbus addr) {temp_addr}  => Reg 2098 (doc) -> {raw_temp}")
        print(f"  Requested (pymodbus addr) {rh_addr}   => Reg 2103 (doc) -> {raw_rh}")
